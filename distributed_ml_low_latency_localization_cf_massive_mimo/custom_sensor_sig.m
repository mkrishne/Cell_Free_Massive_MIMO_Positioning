function [xOut, Rx_t, Rx,sv] = custom_sensor_sig(pos_in,Nsnapshots,ang_in,half_angular_spread,array_spacing,varargin)
...
num_ant_elem = size(pos,2);

%Bessel function matrix computation for covariance matrix calculation
eta =2*pi*array_spacing*deg2rad(half_angular_spread)*cosd(ang_in);
elem_spacing_vec = (0:num_ant_elem-1).' - (0:num_ant_elem-1);
J = besselj(0,eta*elem_spacing_vec) + besselj(2,eta*elem_spacing_vec);
...
if nargout > 2
    ...
    L = chol(Rx_t, 'lower');
    finite_samples = L * (randn(N_ele, Nsnapshots) + 1j*randn(N_ele, Nsnapshots))/sqrt(2);
    R_practical = (1 / Nsnapshots) * (finite_samples * finite_samples');
    Rx = R_practical;
