rng(8);
diary Fig2_positioning_across_N;
...
N_max = length(N); %sweep from 3 antenna per AP to 50 antenna per AP
array_spacing = 0.5;
...
avg_positioning_err_rss_gpr = zeros(1,N_max);
avg_positioning_err_aoa_gpr = zeros(1,N_max);
avg_positioning_err_hybrid_gpr = zeros(1,N_max);
avg_positioning_err_rss_LR_dB = zeros(1,N_max);
avg_positioning_err_rss_WKNN_dB = zeros(1,N_max);

%-----------------------------------------------------------
per_setup_positioning_err_rss_gpr = zeros(nbrOfSetups,num_tp_points);
per_setup_positioning_err_aoa_gpr = zeros(nbrOfSetups,num_tp_points);
per_setup_positioning_err_hybrid_gpr = zeros(nbrOfSetups,num_tp_points);
per_setup_positioning_err_rss_LR_dB = zeros(nbrOfSetups,num_tp_points);
per_setup_positioning_err_rss_WKNN_dB = zeros(nbrOfSetups,num_tp_points);
...
for n_idx = 1:N_max
    ...
    rss_gpr_X = array2table([RSS_fngprnt_dB,x_coordinates],"VariableNames",[RSS_var_names "x_coordinates"]);
    rss_gpr_x_model = fitrgp(rss_gpr_X,"x_coordinates",'kernelfunction',kernel_function);
    rss_gpr_Y = array2table([RSS_fngprnt_dB,y_coordinates],"VariableNames",[RSS_var_names "y_coordinates"]);
    rss_gpr_y_model = fitrgp(rss_gpr_Y,"y_coordinates",'kernelfunction',kernel_function);

    aoa_gpr_X = array2table([nom_azi_angle_fngprnt,x_coordinates],"VariableNames",[AoA_var_names "x_coordinates"]);
    aoa_gpr_x_model = fitrgp(aoa_gpr_X,"x_coordinates",'kernelfunction',kernel_function);
    aoa_gpr_Y = array2table([nom_azi_angle_fngprnt,y_coordinates],"VariableNames",[AoA_var_names "y_coordinates"]);
    aoa_gpr_y_model = fitrgp(aoa_gpr_Y,"y_coordinates",'kernelfunction',kernel_function);
   ...
   %==========================================================================
    for TP_idx = 1:num_tp_points 
        ...
        [x_coordinate_tp,~] = predict(rss_gpr_x_model, per_tp_RSS);
        [y_coordinate_tp,~] = predict(rss_gpr_y_model, per_tp_RSS);
        RSS_GPR_PRED = [x_coordinate_tp y_coordinate_tp];
        dist_coords = [RSS_GPR_PRED;TRUE_TP_COORDS];
        per_setup_positioning_err_rss_gpr(setup_idx,TP_idx) = pdist(dist_coords,'euclidean');

        [x_coordinate_tp,~] = predict(aoa_gpr_x_model, per_tp_est_angles);
        [y_coordinate_tp,~] = predict(aoa_gpr_y_model, per_tp_est_angles);
        AOA_GPR_PRED = [x_coordinate_tp y_coordinate_tp];
        dist_coords = [AOA_GPR_PRED;TRUE_TP_COORDS];
        per_setup_positioning_err_aoa_gpr(setup_idx,TP_idx) = pdist(dist_coords,'euclidean');
        
        [x_coordinate_tp,x_coordinate_sd] = predict(hybrid_gpr_x_model, [per_tp_RSS per_tp_est_angles]);
        [y_coordinate_tp,y_coordinate_sd] = predict(hybrid_gpr_y_model, [per_tp_RSS per_tp_est_angles]);
        HYBRID_GPR_PRED = [x_coordinate_tp y_coordinate_tp];
        dist_coords = [HYBRID_GPR_PRED;TRUE_TP_COORDS];
        per_setup_positioning_err_hybrid_gpr(setup_idx,TP_idx) = pdist(dist_coords,'euclidean');
        ...
        per_setup_positioning_err_rss_LR_dB(setup_idx,TP_idx) = pdist(dist_coords,'euclidean');  
        ...
        per_setup_positioning_err_rss_WKNN_dB(setup_idx,TP_idx) = pdist(dist_coords,'euclidean');  
    end %TP_idx = 1:num_tp_points 

    end %setup_idx = 1:nbrOfSetups

    % Average positioning errors 
    avg_positioning_err_rss_gpr(n_idx) = mean(per_setup_positioning_err_rss_gpr(~isnan(per_setup_positioning_err_rss_gpr) & ~isinf(per_setup_positioning_err_rss_gpr)));
    avg_positioning_err_aoa_gpr(n_idx) = mean(per_setup_positioning_err_aoa_gpr(~isnan(per_setup_positioning_err_aoa_gpr) & ~isinf(per_setup_positioning_err_aoa_gpr)));
    avg_positioning_err_hybrid_gpr(n_idx) = mean(per_setup_positioning_err_hybrid_gpr(~isnan(per_setup_positioning_err_hybrid_gpr) & ~isinf(per_setup_positioning_err_hybrid_gpr)));
    avg_positioning_err_rss_LR_dB(n_idx) = mean(per_setup_positioning_err_rss_LR_dB(~isnan(per_setup_positioning_err_rss_LR_dB) & ~isinf(per_setup_positioning_err_rss_LR_dB)));
    avg_positioning_err_rss_WKNN_dB(n_idx) = mean(per_setup_positioning_err_rss_WKNN_dB(~isnan(per_setup_positioning_err_rss_WKNN_dB) & ~isinf(per_setup_positioning_err_rss_WKNN_dB)));
    
    % Print average positioning errors
    fprintf('\nAverage Positioning Errors:\n');
    
    fprintf('RSS GPR: %.4f\n', avg_positioning_err_rss_gpr(n_idx));
    fprintf('AOA GPR: %.4f\n', avg_positioning_err_aoa_gpr(n_idx));
    fprintf('Hybrid GPR: %.4f\n', avg_positioning_err_hybrid_gpr(n_idx));
    fprintf('RSS LR: %.4f\n', avg_positioning_err_rss_LR_dB(n_idx));
    fprintf('RSS WKNN: %.4f\n', avg_positioning_err_rss_WKNN_dB(n_idx));
